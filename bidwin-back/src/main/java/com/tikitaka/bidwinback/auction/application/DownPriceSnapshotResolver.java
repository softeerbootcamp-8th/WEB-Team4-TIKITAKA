...
public class DownPriceSnapshotResolver {
    ...
    public boolean supports(AuctionListQuery query) {
        return query.auctionType() == AuctionType.DOWN
                ...
                && query.keyword() == null
                && query.category() == null
                && query.size() == AuctionListService.DEFAULT_PAGE_SIZE
                && query.page() >= 1
                && query.page() <= AuctionListService.MAX_LIST_PAGES;
    }
    ...
    private DownPriceSnapshotPage slice(
        ...
        int fromIndex = Math.min(
                (page - 1) * AuctionListService.DEFAULT_PAGE_SIZE,
                entries.size()
        );
        int toIndex = Math.min(
                fromIndex + AuctionListService.DEFAULT_PAGE_SIZE,
                entries.size()
        );
