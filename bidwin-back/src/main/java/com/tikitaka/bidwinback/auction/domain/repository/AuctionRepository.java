...
public interface AuctionRepository extends JpaRepository<Auction, Long> {
    ...
    // startedAt <= asOf까지 걸어야, asOf 스냅샷을 공유하는 다음 페이지 요청 사이에 새로
    // 등록된 경매가 끼어들어 목록 구성이 흔들리는 걸 막을 수 있다.
    // createdAt이 아닌 startedAt을 쓰는 이유: createdAt은 애플리케이션 서버(JVM) 시계로 찍히는
    // 반면 asOf/endedAt은 DB 시계(current_timestamp)로 찍힌다. 두 시계가 어긋나면(특히 타임존)
    // 방금 생성한 경매의 createdAt이 asOf보다 미래로 찍혀 조건을 만족 못 하고 목록에서 통째로
    // 빠질 수 있다. startedAt은 createdAt과 같은 의미(행 생성 시각)이면서 DB 시계로 찍히므로 안전하다.
