...
<table align="center" width="100%">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <ul>
          <li><b>경매 목록 조회 성능 최적화</b> — QueryDSL 기반 DB 페이지네이션, 정렬별 Top-K 후보 조회, 복합 인덱스·입찰 수 반정규화로 OOM을 제거하고 부하 테스트 기준 500 RPS달성</li>
            <li><b>상향 경매 입찰·거래 흐름 고도화</b> — 조건부 UPDATE 기반 입찰 동시성 제어, 밀봉 입찰·초기 낙찰 판정, 거래 동의 API, 비낙찰자 보증금 반환 및 마감 배치 실패 격리 구현</li>
          <li><b>S3 이미지 업로드 생명주기</b> — Presigned URL 직접 업로드, uploadId 기반 소유권·무결성 검증, 임시 객체 영구 승격, 미사용 이미지 정리와 트랜잭션 보상 처리 구현</li>
          <li><b>백엔드 공통 기반 및 회원 보안</b> — 공통 API 응답·전역 예외 처리, PBKDF2 비밀번호 암호화, 회원정보 유일성 보장, 일회용 재설정 토큰과 메일 발송 제한·비동기 처리 구현</li>
        </ul>
      </td>
      <td align="center">
    ...
    <tr>
      ...
      <td>
        <ul>
          <li><b>상향 입찰 동시성 병목 규명 및 Redis 원자적 필터 구현</b> — 부하 테스트로 처리량이 약 240 RPS에서 포화하는 원인이 커밋까지 유지되는 행 잠금임을 규명. 세션 내 직렬화, 분산락·메시지 큐 등 대안을 비교한 뒤 Redis Lua 기반 원자적 가격 필터를 구현하고, 필터로서 해소되지 않는 한계를 확인해 판정 이관 설계로 연결</li>
    <li><b>동시 거래 경로의 교착 대응</b> — 보증금 정산·입찰·즉시구매가 회원 행과 경매 행을 서로 반대 순서로 잠그는 구조에서 발생하는 순환 대기를 정리. 정산 경로는 회원 ID 기준으로 잠금 순서를 고정해 교착을 원천 차단하고, 순서를 통일할 수 없는 경로는 쿼리 타임아웃을 짧게 두고 예외를 동시성 충돌 응답으로 변환해 장애 전파를 격리</li>
    <li><b>세션 저장소 Redis 전환</b> — 다중 인스턴스 환경을 위해 세션 스토리지를 Redis로 이관하고, 캐시 장애와 세션 역직렬화 실패 등 예외 상황의 인증 처리 흐름을 정리</li>
    <li><b>프로젝트 기반 및 인프라 초기 구축</b> — AWS 인프라 및 CD 설정, 백엔드·프론트엔드 초기 세팅, 커밋·PR 린트 컨벤션 정립</li>
        </ul>
      </td>
      <td align="center">
