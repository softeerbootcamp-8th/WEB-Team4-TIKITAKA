...
export function useDownAuctionClock(pricing: DownPricing, serverOffsetMs = 0) {
  const [state, setState] = useState(() => computeState(pricing, serverOffsetMs))

  useEffect(() => {
    const tick = () => setState(computeState(pricing, serverOffsetMs))
    tick()
    const id = setInterval(tick, TICK_INTERVAL_MS)
    return () => clearInterval(id)
  }, [pricing, serverOffsetMs])
